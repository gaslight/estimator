defmodule EstimationTest do
  ...
  test "adding two estimates that disagree", %{story: story, project: project, estimation: estimation} do
    [team_member1, team_member2] = project.team_members
    estimation = estimation
    |> Estimation.join_estimation(team_member1)
    |> Estimation.join_estimation(team_member2)
  ...
  test "adding two estimates that agree", %{story: story, project: project, estimation: estimation} do
    [team_member1, team_member2] = project.team_members
    estimation = estimation
    |> Estimation.join_estimation(team_member1)
    |> Estimation.join_estimation(team_member2)
    |> Estimation.estimate(team_member1, 3)
    |> Estimation.estimate(team_member2, 3)
    assert estimation.state == :agree
    assert estimation.estimate == 3
  end
